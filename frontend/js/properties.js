...
function _buildSlideWorkspacePanel(panel) {
                ...
                ${
                    activeThemeSystem
                        ? `<div class="sf-theme-intel">
                            <span>${escapeHtml(activeThemeSystem.family)}</span>
                            <span>${escapeHtml(activeThemeSystem.intent?.tone || "presentation")}</span>
                            <span>${escapeHtml(activeThemeSystem.tokens?.density?.default || "standard")}</span>
                        </div>
    ...
    layoutGrp.innerHTML += `
        <div class="space-y-3 sf-preset-browser-shell">
            ...
            <div class="text-xs text-slate-600">Recommended presets react to current slide content, selected theme, and recent choices.</div>
        </div>
    `;
